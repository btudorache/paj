...
public class Test1 {
	...
	public void testBank() throws ClientExistsException {
        ...
        ArrayList<Client> clientArray = new ArrayList<>(clientSet);
		System.out.println(clientArray.get(0).toString());
		System.out.println(clientArray.get(1).toString());

		assertEquals(2, bank.getClients().size());
		assertEquals("Mr. Smith John", clientArray.get(1).getClientGreeting());
		assertEquals("Mr. Smith John", clientArray.get(1).toString());
		assertEquals("Ms. Smith Michelle", clientArray.get(0).getClientGreeting());
		assertEquals("Ms. Smith Michelle", clientArray.get(0).toString());
	}
